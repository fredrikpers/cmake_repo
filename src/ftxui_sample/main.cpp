...
// configuration step. It creates a namespace called `cmake_repo`. You can modify
...
{
  try {
    CLI::App app{ fmt::format("{} version {}", cmake_repo::cmake::project_name, cmake_repo::cmake::project_version) };
    ...
    if (show_version) {
      fmt::print("{}\n", cmake_repo::cmake::project_version);
      return EXIT_SUCCESS;
    }
